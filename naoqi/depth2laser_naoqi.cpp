...
void visualizeLaser(const std::vector<float>& ranges){
  ...
  for (int i=0; i<ranges.size(); i++){
    ...
    if(ranges[i]<laser_conf.range_max+0.1){
      float x, y;
...
void publishLaser(const qi::AnyObject memory_service, const std::vector<float>& ranges){
  //TODO: publish information in the following ALMemory Keys:
  //NAOqiDepth2Laser/Ranges
  //NAOqiDepth2Laser/NumRanges
  //NAOqiDepth2Laser/MinAngle
  //NAOqiDepth2Laser/MaxAngle
...
void depth2laser_thread(bool read_disk, ALVideoDeviceProxy camProxy, std::string clientName, int frame, Eigen::Isometry3f laser_tf, bool use_gui, bool remote, qi::AnyObject motion_service, qi::AnyObject memory_service){
  ...
  while (!stop_thread){
    ...
    /*if(read_disk)
      ...
      getImageFromCamera(camProxy, clientName, imgHeader);
    */
    if(!read_disk)
    {
      if(!remote)
    ...
    //TODO: publishLaser(memory_service, ranges);
    publishLaser(memory_service, ranges);

    qi::AnyObject _subscriber_pose;
    _subscriber_pose = memory_service.call<qi::AnyObject>("subscriber", "NAOqiDepth2Laser/Ranges");
...
int main(int argc, char **argv)
{
  ...
  description.add_options()
    ...
    ("laser_plane_thickness", po::value<double>()->default_value(0.1), "Laser plane thickness to consider points from depth image.")
